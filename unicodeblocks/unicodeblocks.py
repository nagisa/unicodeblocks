...
class Blocks(object):
    ...
    For example len(Blocks(...)) will return ammount of characters in all
    blocks instead of ammount of blocks inside object.
    """
    ...
    def __getitem__(self, key):
        return self._names.__getitem__(self._normalize_name(key))
    ...
    def __repr__(self):
        if len(self) > 10:
            return "Blocks(...{0} * Block...)".format(len(self._names))
        else:
            args = ','.join(map(repr, self.blocks()))
...
if __name__ == "__main__":
    ...
    class BlocksTests(unittest.TestCase):
        ...
        def test_block_of(self):
            self.assertEqual(blocks.block_of('a'), blocks.block_of('b'))
            self.assertNotEqual(blocks.block_of('a'), blocks.block_of('ą'))
            self.assertEqual(blocks.block_of(chr(0)),
                             blocks.block_of(chr(127)))
            self.assertNotEqual(blocks.block_of(chr(127)),
                                blocks.block_of(chr(128)))
            with self.assertRaises(TypeError):
                blocks.block_of(1)
            with self.assertRaises(TypeError):
                blocks.block_of("hello")

        def test_blocks(self):
            self.assertSequenceEqual(next(blocks.blocks()),
                                     next(blocks.blocks()))
            self.assertSequenceEqual(list(blocks.blocks()),
                                     list(blocks.blocks()))
            self.assertEqual(next(blocks.blocks()), blocks['Basic Latin'])
